  ...
  <view class="market-container">
    ...
    >
      <view class="menu-scroll">
        <view
          v-for="(item, idx) in navTabs"
          :key="idx"
          class="menu-item"
          :class="{ active: idx === activeNav }"
    ...
    >
      ...
      <view id="content-start" v-if="activeNav === 0" class="follow-content">
        ...
        <view class="profile-section">
          <view class="profile-row">
            ...
            <view class="profile-info">
              <view class="name-row">
                <text class="name">{{ $t('market.traderName') }}</text>
              </view>
          ...
          <view class="stats-row">
            <view class="stat-item">
              <text class="stat-number">2314</text>
          ...
          <view class="badge-row">
            <view class="badge">
                <image
                  class="DividendsIcon"
                  src="/static/icons/DividendsIcon.png"
                />{{ $t('market.profitShareRate') }}
              </view>
          </view>
        </view>
        ...
        <view class="content-tabs">
          <view class="tab-item" :class="{ active: activeContentTab === 'auto' }" @click="activeContentTab = 'auto'">
            {{ $t('market.autoFollow') }}
          </view>
          <view class="tab-item" :class="{ active: activeContentTab === 'views' }" @click="activeContentTab = 'views'">
          ...
          <view class="trader-card">
            <view class="trader-info">
              ...
              <view class="trader-details">
                <text class="trader-name">{{ $t('market.traderName') }}</text>
                <text class="trader-rank">
                  <image
                    class="humanIcon"
                    src="/static/icons/humanIcon.png"
                  /><text class="rankTitle">70</text>/550
                </text>
              </view>
            ...
            <view class="trader-stats-overview">
              <view class="stat-item">
                ...
                <text class="stat-value green">101.86%</text>
              </view>
              <view class="stat-item">
                <text class="stat-label">{{ $t('market.winRate') }}</text>
                <text class="stat-value">68.5%</text>
              </view>
              <view class="stat-item">
          ...
          <view class="FollowAmountBox">
            <view class="follow-amount-section">
              <view class="section-header">
                ...
                <text class="available-balance">{{ $t('market.available') }}: 10,000USDT</text>
              </view>
                  ...
                  <template v-if="selectedAmount === 'custom'">
                    <view class="custom-input-wrapper">
                      <input
                        ...
                        type="number"
                        :placeholder="$t('market.inputAmount')"
                        inputmode="decimal"
                        @click.stop
                        @input="validateCustomAmount($event)"
                        min="0"
                        step="0.01"
                      />
                ...
                <view class="dropdown-options" v-if="toggleDropdown">
                  ...
                  >
                    ...
                    <image
                      ...
                      src="/static/icons/checkIcon.png"
                      mode="aspectFit"
                    />
                  </view>
          ...
          <view class="trading-stats-section">
            ...
            <view class="trading-stats-grid">
              <view class="trading-stat-item">
                ...
                <text class="trading-stat-value">156</text>
              </view>
              <view class="trading-stat-item">
                <text class="trading-stat-label">{{ $t('market.avgHoldingTime') }}</text>
                <text class="trading-stat-value">4.2h</text>
              </view>
              <view class="trading-stat-item">
                <text class="trading-stat-label">{{ $t('market.maxDrawdown') }}</text>
                <text class="trading-stat-value redLebal">63.74%</text>
              </view>
              <view class="trading-stat-item">
                <text class="trading-stat-label">{{ $t('market.managedAssets') }}</text>
                <text class="trading-stat-value">2113.252.27</text>
              </view>
            </view>
          </view>
          ...
          <view class="risk-warning">
            <view class="risk-title">
              <text class="risk-icon">
                <image
                  class="warnWhite"
                  src="/static/icons/warnWhite.png"
                  mode="aspectFit"
                />
              </text>
          ...
          <view class="profit-preview">
            <view class="preview-box">
              <view class="preview-left">
                ...
                <text class="profit-amount green">+1098.60USDT</text>
                <text class="profit-calculation"
                    >{{ $t('market.expectedReturnDesc') }}</text
                  >
              </view>
              <view class="profit-right">
                <image
                  class="timeIcon"
                  src="/static/icons/timeIcon.png"
                  mode="aspectFit"
                />
              </view>
            </view>
        ...
        <view
          v-else-if="activeContentTab === 'views'"
          class="personal-views-content"
        >
          <!-- 使用v-for渲染view-item -->
          <view class="view-item" v-for="item in viewItems" :key="item.id">
            <view class="view-header">
              ...
              <view class="user-info">
                ...
                <view class="follow-btn-mini" :class="{ 'followed-btn-mini': item.followed }" @click="toggleItemFollow(item.id)">
            ...
            <view class="actions-box">
              <view class="view-actions">
                ...
                <view class="action-item">
                  <image
                    src="/static/icons/CommentIcon.png"
                    class="commentIcon"
                  />
            ...
            <view
              ...
              v-if="item.commentsList && item.commentsList.length > 0"
            >
              <view
                class="comment-item"
                v-for="comment in item.commentsList"
                :key="comment.id"
              >
      ...
      <view
        ...
        id="content-start"
        class="market-data-tab"
      >
        <view class="page-stock">
          <!-- List - 港股数据 -->
          <view class="results-list">
            <view
              v-for="(item, index) in getMarketData('hk')"
              :key="index"
              class="result-item"
            ...
            >
              <view class="item-left">
                ...
                <image
                  :src="item.sparkline || '/static/icons/line_chart.png'"
      ...
      >
        <view class="page-stock">
          ...
          <view class="results-list">
            <view
              v-for="(item, index) in getMarketData('spot')"
              :key="index"
              class="result-item"
              @click="handleMarketItemClick(item)"
            >
      ...
      >
        <view class="page-stock">
          ...
          <view class="results-list">
            <view
              v-for="(item, index) in getMarketData('contract')"
              :key="index"
              class="result-item"
              @click="handleMarketItemClick(item)"
            >
      ...
      >
        <view class="page-stock">
          ...
          <view class="results-list">
            <view
              v-for="(item, index) in getMarketData('xcoin')"
              :key="index"
              class="result-item"
              @click="handleMarketItemClick(item)"
            >
            ...
            <view class="contentbox">
              <view class="coin-selector">
                ...
                <view class="right-box">
                  <view class="coin-code"
                    >{{ selectedFromCoin.code }}
                    <image
                      class="dropdown-arrow"
                      src="/static/icons/arrow-bottom.png"
                      mode="aspectFit"
