...
import { useUserStore, TickersParams, AirdropsParams } from "@/store/modules/user";
...
onLoad(async (option) => {
	try {
		...
		if (bannersResponse.data.status === -1){
			handleLogout()
			return
		}
	...
	try {
		...
		if (resultAirdrops?.data?.data) {
			listAirdrops.value = resultAirdrops.data.data;
		} else if (Array.isArray(resultAirdrops?.data)) {
			listAirdrops.value = resultAirdrops.data;
		} else {
			listAirdrops.value = [];
		}

		console.log('Airdrops summary loaded:', listAirdrops.value);
